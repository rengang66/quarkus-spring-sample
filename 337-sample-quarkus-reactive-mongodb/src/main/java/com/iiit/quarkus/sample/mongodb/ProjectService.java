...
public class ProjectService {
	...
	private void initDBdata() {
		deleteAll();
		Project project1 = new Project("项目A", "关于项目A的描述1");
		Project project2 = new Project("项目B", "关于项目B的描述");
		
		Document document1 = new Document().append("name", project1.name).append(
				"description", project1.description);
		
		Document document2 = new Document().append("name", project2.name).append(
				"description", project2.description);
		getCollection().insertOne(document1).onItem().ignore().andContinueWithNull().await().indefinitely();
		getCollection().insertOne(document2).onItem().ignore().andContinueWithNull().await().indefinitely();		
	...
	public Uni<List<Project>> get( String name) {
		// return getCollection().find()
		   return getCollection().find(Filters.eq("name", name))				   
				   .map(doc -> {
	                	Project project = new Project(doc.getString("name"),
	                			doc.getString("description"));	                	
	                    return project;
	...
	public Uni<Void> update(Project project) {	
		Document document = new Document().append("name", project.name).append(
				"description", project.description);	
		return getCollection().replaceOne(Filters.eq("name", project.name),document)
	...
	private void deleteAll() {
		...
		getCollection().deleteMany(document).await().indefinitely();
	}	
